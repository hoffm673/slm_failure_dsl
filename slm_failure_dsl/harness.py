...
Key outputs:
  fire_counts        : (step, failure_mode) -> firing count
  declared_coverage  : fraction of declared step-level modes that fired
  leakage_rate       : fraction of step-level firings that were undeclared
  inconsistency_rate : fraction of (step, seed) combos with inconsistent reruns

latent_inconsistency is a HARNESS-LEVEL mode (measured across reruns of the
same input, not per-step). It's excluded from coverage and leakage so those
metrics measure step-level reliability honestly.
"""
...
class AuditReport:
    ...
    def declared_coverage(self) -> float:
        """Fraction of declared step-level modes that fired at least once."""
        total = 0
        covered = 0
    ...
    def inconsistency_rate(self) -> float:
        """Fraction of (step, seed) combos where reruns disagreed."""
        if self.n_seeds == 0:
            return 0.0
